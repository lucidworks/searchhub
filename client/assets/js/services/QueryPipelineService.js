...
(function () {
  ...
  function QueryPipelineService(QueryDataService, ConfigService, QueryBuilder, $http, $q, ApiBase, $log) {
    'ngInject';
    ...
    var service = {
      ...
      cfRecsQueryPipeline:cfRecsQueryPipeline
    };
    return service;
    ...
    // Recs Query Pipelines - these pipelines hit the recs endpoint
    function cfRecsQueryPipeline(queryObject, pipeline) {
        $log.info("We are generating Collaborative Filtering Recs. We will be using pipeline" + pipeline)
        var queryString = QueryBuilder.objectToURLString(queryObject);
        var theUrl = ApiBase.getEndpoint() + 'recs/pipeline=' + pipeline + "/path=" + queryString;
        return doQuery(theUrl)
    }
  }
})();