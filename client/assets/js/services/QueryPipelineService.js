...
(function () {
  ...
  function QueryPipelineService(QueryDataService, ConfigService, QueryBuilder, $http, $q, ApiBase, $log) {
    ...
    var service = {
      ...
      recsQueryPipelineWithCollection:recsQueryPipelineWithCollection,
    ...
    function cfRecsQueryPipeline(queryObject) {
        console.log("We are in the cf recs query pipeline. We will be using collection lucidfind and pipeline cf-recs")
        var queryString = QueryBuilder.objectToURLString(queryObject);
        var theUrl = ApiBase.getEndpoint() + 'recs/' + queryString;
        return doQuery(theUrl)
    }

    function recsQueryPipelineWithCollection(collection, queryObject, pipelineName){
      console.log("We are in the recs query pipeline with collection for the collection", collection)
      var queryString = QueryBuilder.objectToURLString(queryObject);
      var theUrl = ApiBase.getEndpoint() + 'recs/query-pipelines/' +
          pipelineName + '/collections/' + collection +
          '/select?' + queryString;
      $log.info("the url generated by recsquerypipeline with collection is as follows", theUrl)
      return doQuery(theUrl);
    }

    function recsQueryPipeline(queryObject, pipelineName) {
      console.log("We are in the recs query pipeling without collection")
      var queryString = QueryBuilder.objectToURLString(queryObject);
      var theUrl = getRecQueryUrl(pipelineName) + '?' + queryString;
      $log.info("the url generated by recsquerypipeline is as follows", theUrl)
